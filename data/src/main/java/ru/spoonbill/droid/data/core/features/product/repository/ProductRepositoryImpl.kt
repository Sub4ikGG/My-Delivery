...
internal class ProductRepositoryImpl(
    ...
    override suspend fun getPopularProductsFlow(): Flow<List<Product>> = dao.getByCategoryId(7).map{ list -> list.map(ProductEntity::toModel) }

    override suspend fun getRecommendedProductsFlow(): Flow<List<Product>> = dao.getByCategoryId(7).map{ list -> list.map(ProductEntity::toModel) }

    override suspend fun getActualProductsFlow(): Flow<List<Product>> = dao.getByCategoryId(7).map{ list -> list.map(ProductEntity::toModel) }

    override suspend fun getPromotions(
        dispatcher: CoroutineDispatcher
    ): List<Promotion> = withContext(dispatcher) {
        return@withContext remoteDataSource.getPromotions()
    }
    override suspend fun getProductsByCategoryId(categoryId: Long, ): Flow<List<Product>> =
        dao.getByCategoryId(categoryId).map{ list -> list.map(ProductEntity::toModel) }
    ...
    override suspend fun getCartProductsFlow(
        ...
        return@withContext dao.getCartProducts().map { it.map(ProductEntity::toModel) }
    }
